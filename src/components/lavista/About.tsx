...
export function About() {
  ...
  useEffect(() => {
    const channel = supabase
      ...
      .on("postgres_changes", { event: "*", schema: "public", table: "about_images" }, () => {
  ...
  const title = aboutData?.title ?? "A warm corner of Giza, ten minutes from the pyramids.";
  ...
  const heroImg = (images && images.length > 0 ? images[0] : null) ?? resolvedContentImg ?? lounge;
